...
def run_example(example_name, args, timeout=10, expected_return_code=0, requires_root=False):
	command_to_run = (['sudo'] if bool(requires_root) is True and (platform.system() == 'Linux' or platform.system() == 'Darwin') else []) + [os.path.join(DEFAULT_EXAMPLE_DIR, example_name)]
